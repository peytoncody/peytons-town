...
function loadLoginBackground() {
  const loginBg = localStorage.getItem("peytonsLoginBG");
  if (loginBg) {
    document.getElementById("login").style.backgroundImage = `url(${loginBg})`;
    document.getElementById("login").style.backgroundSize = "cover";
    document.getElementById("login").style.backgroundPosition = "center";
  }
}

document.getElementById("loginBgUpload").addEventListener("change", function () {
  const file = this.files[0];
  const reader = new FileReader();
  reader.onload = function () {
    localStorage.setItem("peytonsLoginBG", reader.result);
    document.getElementById("login").style.backgroundImage = `url(${reader.result})`;
    document.getElementById("login").style.backgroundSize = "cover";
    document.getElementById("login").style.backgroundPosition = "center";
  };
...
document.getElementById("stealthToggle").addEventListener("click", function () {
  ...
  const stealthWeather = document.getElementById("stealthWeather");
  const stealthModeBody = document.body;

  const isStealthActive = stealthWeather.style.display === "block";

  if (isStealthActive) {
    // Turn OFF stealth mode
    stealthWeather.style.display = "none";
    diaryContent.style.display = "block";
    settingsPanel.classList.remove("hidden-in-stealth");
    stealthModeBody.classList.remove("stealth-mode");
  } else {
    // Turn ON stealth mode
    diaryContent.style.display = "none";
    stealthWeather.style.display = "block";
    settingsPanel.classList.add("hidden-in-stealth");
    stealthModeBody.classList.add("stealth-mode");
